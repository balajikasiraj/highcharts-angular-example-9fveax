...
export class ChartComponent implements OnInit {
  ...
  addSerie = <T extends Highcharts.IndividualSeriesOptions>(newSerie: T) => {
    const newSeries = this.chartOptions.series
      ? [...this.chartOptions.series, newSerie]
      : [newSerie];
  ...
  exportChartLocal = (type = "image/png", filename = "my-chart") =>
    this.chartObject$
      .pipe(
        first(),
        tap(c => console.log(c.options.chart.type))
      )
      .subscribe(chart =>
        chart.exportChartLocal({ type, filename, sourceWidth: 2048 })
      );
  ...
  private initChart = () => {
    ...
    chartOptions.lang = {
      loading: "Loading ...",
      noData: "No data available, please try again"
    };
